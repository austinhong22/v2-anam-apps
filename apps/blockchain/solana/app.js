...
class SolanaAdapter extends CoinAdapter {
  ...
  async generateWallet() {
    try {
      // 브라우저 호환 번들 전역 사용
      const { bip39, keypairFromMnemonic } = window.solanaWeb3 || {};
      if (!bip39 || !keypairFromMnemonic) {
        throw new Error("Solana 번들 로더를 찾을 수 없습니다");
      }
  ...
  async importFromMnemonic(mnemonic) {
    try {
      // solana-bundle에서 제공하는 bip39/파생 유틸 사용 (브라우저 호환)
      const { bip39, keypairFromMnemonic } = window.solanaWeb3 || {};
      if (!bip39) {
        return Result.failure("bip39 로더를 찾을 수 없습니다");
      }

      // 단어 유효성(워드리스트) 체크 → 어떤 단어가 잘못됐는지 알려줌
      const words = mnemonic.trim().toLowerCase().split(/\s+/);
      const english = (bip39.wordlists && bip39.wordlists.english) || [];
      const invalidWords = words.filter((w) => !english.includes(w));
      if (invalidWords.length > 0) {
        return Result.failure(`BIP39 목록에 없는 단어: ${invalidWords.join(", ")}`);
      }
